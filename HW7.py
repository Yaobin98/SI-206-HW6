...
def make_players_table(data, cur, conn):  
    ...
    positions_dict = {row[0]: row[1] for row in positions_data}
    for player_info in data['squad']:
        player_id = int(player_info['id'])
        name = player_info['name']
        position_name = player_info['position']
        position_id = positions_dict.get(position_name)
        birthyear = int(player_info['dateOfBirth'][:4])
        nationality = player_info['nationality']
        
        cur.execute('''INSERT OR IGNORE INTO Players (id, name, position_id, birthyear, nationality)
                     VALUES (?, ?, ?, ?, ?)''', (player_id, name, position_id, birthyear, nationality))  
    conn.commit()
...
def nationality_search(countries, cur, conn):
    country = ', '.join('?' * len(countries)) 
    cur.execute(f'''
        SELECT name, position_id, nationality
        FROM Players
        WHERE nationality IN ({country})
    ''', countries)
    return cur.fetchall()
